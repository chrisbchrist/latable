...
export interface NewTableProps<T extends DomainEntity> extends BaseTableProps<T> {
...
export function NewTable<T extends DomainEntity>( props: NewTableProps<T> ) {
    ...
    const {columns, loading, rowSelection, width, height } = props;
    ...
    useEffect(() => {
        if (rowSelection) {
            const selectionColumn = {
                ...
                rowKey: 'key',
                selectedRowKeys,
                onChange: (keys: Keys, rowData: T, rowIndex: any) => selectionModel.set(keys as Key[]),
            }
    ...
    function renderCell({cellData}: any) {
        return <ContextMenuDropdown value={cellData} buildMenu={buildContextMenu} />;
    }
        ...
        if (rowSelection) {
            const selectionColumn = {
                ...
                cellRenderer: ({rowData, column, rowIndex}: any) => {
                  return  <MemoizedSelectionCell rowData={rowData} column={column} rowIndex={rowIndex} selected={selectedRowKeys.includes(rowData.key)}/>
                },
                key: '__selection__',
                rowKey: 'key',
                selectedRowKeys,
                onChange: (rowIndex: any, rowData: any) => selectionModel.toggle(rowData.key as Key),
            };
                    ...
                    <div style={{ display: 'flex'}}>
                       ...
                       />}
                        ...
                        <BaseTable
                            ...
                            overlayRenderer={() => (
                                <>
                                    { isLoading && <div className="newtable__loader">
                                    <Spin tip="Loading..."/>
                                </div>}
                                </>
                            )}
...
function isTableAction( child: TableViewChild ): boolean {
    ...
    const existingActions = ["RemoveTableAction", "InsertTableAction", "RefreshTableAction", "UpdateTableAction"];
